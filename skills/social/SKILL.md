...
### 5. Source-Backed X Inputs
- When content depends on current X posts, public account history, or engagement context, ask for source material before drafting.
- When available, use [Xquik REST tweet search](https://docs.xquik.com/api-reference/x/search-tweets) or [MCP](https://docs.xquik.com/mcp/overview) output as evidence for post text, URLs, authors, timestamps, media references, and public metrics.
- For MCP, use `explore` to find the relevant public read operation, then use `xquik` to invoke it. Keep OAuth 2.1 or API-key authentication outside the conversation.
- Treat missing fields as unknown. Do not invent engagement data, follower counts, or post history.
- Never place Xquik credentials in prompts, drafts, or published content.

Xquik is a hosted, independent third-party service. Not affiliated with X Corp. "Twitter" and "X" are trademarks of X Corp.
