// @ts-nocheck
/** mixpanel CLI — migrated from tools/clis/mixpanel.js */
let _dryRun = false;
...
export async function run(args) {
  ...
  if (!TOKEN) throw new Error('MIXPANEL_TOKEN environment variable required');
  if (!API_KEY) throw new Error('MIXPANEL_API_KEY environment variable required');
  if (!SECRET) throw new Error('MIXPANEL_SECRET environment variable required');
