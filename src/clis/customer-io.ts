// @ts-nocheck
/** customer-io CLI — migrated from tools/clis/customer-io.js */
let _dryRun = false;
...
export async function run(args) {
  ...
  if (!APP_KEY) throw new Error('CUSTOMERIO_APP_KEY environment variable required');
  if (!SITE_ID) throw new Error('CUSTOMERIO_SITE_ID environment variable required');
  if (!API_KEY) throw new Error('CUSTOMERIO_API_KEY environment variable required');
