// @ts-nocheck
/** adobe-analytics CLI — migrated from tools/clis/adobe-analytics.js */
let _dryRun = false;
...
export async function run(args) {
  ...
  if (!ACCESS_TOKEN) throw new Error('ADOBE_ACCESS_TOKEN environment variable required');
  if (!CLIENT_ID) throw new Error('ADOBE_CLIENT_ID environment variable required');
  if (!COMPANY_ID) throw new Error('ADOBE_COMPANY_ID environment variable required');
