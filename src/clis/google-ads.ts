// @ts-nocheck
/** google-ads CLI — migrated from tools/clis/google-ads.js */
let _dryRun = false;
...
export async function run(args) {
  ...
  if (!TOKEN) throw new Error('GOOGLE_ADS_TOKEN environment variable required');
  if (!DEV_TOKEN) throw new Error('GOOGLE_ADS_DEVELOPER_TOKEN environment variable required');
  if (!CUSTOMER_ID) throw new Error('GOOGLE_ADS_CUSTOMER_ID environment variable required');
